...
exports.token2user = async (req) => {
  try {
    ...
    if (config.NoAuthApis.includes(func)) {
      isParkFun = true;

    }
    let token_str = req.get('authorization');
    if(!isParkFun || token_str) {
      let token_str = req.get('authorization');
      if (!token_str) {
      ...
      logger.info('NoAuthApi:'+func);
      return 0;
    }
    logger.info('unknow reason:' +token_str);
    return -1;
  } catch (error) {
    logger.error(error);
    return -1;
...
exports.supppliertoken = (type, user, identifyCode, magicNo) => {
  try {
    let expires = ''
    if (type === 'MOBILE') {
      expires = Date.now() + config.MOBILE_TOKEN_AGE;
    ...
    let s = [type, user.park_supplier_id, identifyCode, expires.toString(), config.SECRET_KEY].join('-');
    let L = [type, user.park_supplier_id, magicNo, expires.toString(), CryptoJS.SHA1(s).toString()]
    return L.join('-')
  } catch (error) {
    logger.error(error);
    return null;
