...
function checkDate(dateStr) {
    ...
    //验证格式是否正确
    const dataFormat = /^(\d{4})-(\d{2})-(\d{2})$/;
    return dataFormat.test(dateStr);
}
...
function sendErcError(res, exception) {
...
function sendErrorDefined(res, error) {
    let sendData = {
        errno: -1,
        error
    };
        ...
        if (!fs.existsSync(dirpath)) {
            ...
            dirpath.split(/[/\\]/).forEach(function (dirname) { //这里指用/ 或\ 都可以分隔目录  如  linux的/usr/local/services   和windows的 d:\temp\aaaa
                if (dirname) {
                    ...
                    if(pathtmp.substring(0,1)=== '/' && dirpath.substring(0,1) !== '/'){
...
async function queryPure(db, req, queryStr, replacements) {
    let queryRst = await db.query(queryStr, {
        replacements,
        type: db.QueryTypes.SELECT
    })
...
async function queryWithCount(db, req, queryStr, replacements) {
    ...
    return {
        count: count.length > 0 ? count[0].num : 0,
        data: queryRst
    }
}
...
async function queryWithGroupByCount(db, req, queryStr, replacements) {
    ...
    let queryRst = await db.query('select * from (' + queryStr + ') as a LIMIT ?,?', {
        replacements: rep,
        type: db.QueryTypes.SELECT
    })
...
function excelToJson(uploadurl) {
    ...
    const tempfile = path.join(__dirname, '../' + config.uploadOptions.uploadDir + '/' + fileName);
...
async function mrpLog(fileName, writeStr) {
    fs.appendFileSync(path.join(__dirname, config.mrpDir + fileName), writeStr, function (err) {
        if (err) {
            logger.err(err)
            return
        }
...
module.exports = {
    ...
    sendErrorDefined: sendErrorDefined,
    checkDate: checkDate,
    isRepeat: isRepeat,
    mrpLog: mrpLog,
    sendErcError
