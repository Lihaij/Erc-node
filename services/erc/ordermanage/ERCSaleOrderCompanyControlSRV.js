...
let search_sales = async (req, res) => {
    try {
        let doc = common.docTrim(req.body),
            ...
            returnData = {},
            replacements = [];
        let queryStr = `select o.*,d.domain_name,d.domain_contact,d.domain_phone,u.username,u.phone,u.name,c.corporateclients_name,c.corporateclients_contact_phone  
         from tbl_erc_order o
        ...
        for (let r of result.data) {
            let result = JSON.parse(JSON.stringify(r));
            let delivery = await tb_delivery.findAll({
                where: {
                    state: 1,
                    order_id: result.order_id
                }
            })
            let delivery_date_temp = ''
            for (let d of delivery) {
                let elem = d.delivery_date ? moment(d.delivery_date).format("YYYY-MM-DD") : ''
                delivery_date_temp = delivery_date_temp + elem + ' | '
            }
            if (delivery_date_temp) {
                delivery_date_temp = delivery_date_temp.substr(0, delivery_date_temp.length - 3)
            }
            result.delivery_date = delivery_date_temp
...
let initAct = async (req, res) => {
    let returnData = {},
        queryStr, queryStr2, replacements = [];
    let user = req.user
    returnData = {
    ...
    returnData.unitInfo = await getBaseType('JLDW', user.domain_id)
...
let addAct = async (req, res) => {
    try {
        ...
        //创建会计科目详情
        await ERCAccountingControlSRV.addAccountingDetail(5001, addOrder.order_id, user.domain_id);
        common.sendData(res, retData);
    } catch (error) {
        common.sendFault(res, error);
    }
...
async function getQrcodeAct(req, res) {
    let doc = common.docTrim(req.body);
    let svg_string = qr.imageSync(doc.order_id, {
        type: 'svg'
    });
    common.sendData(res, {
        qrcode: svg_string
    ...
    try {
        ...
        if (doc.clientType == 1) { //机构
            ...
            let addNcaOrder = await tb_order.create({
                ...
                biz_code: await genBizCode(CODE_NAME.SO, user.domain_id, 6)
            ...
            //创建会计科目详情
            await ERCAccountingControlSRV.addAccountingDetail(5001, addNcaOrder.order_id, user.domain_id);
            ...
            let addNcaOrder = await tb_order.create({
                ...
                sales_data_source: 2, //标识该采购单来源 1mrp运算，2手动添加
                sap_order_state: 1, //标识该销售单sap状态'
                order_review_state: 0, //订单评审标志  0未提交 1已提交2 2通过 3拒绝
                biz_code: await genBizCode(CODE_NAME.SO, user.domain_id, 6)
            });
...
async function getMaterielTemp(req, res) {
    try {
        ...
        let orporateclients = await tb_corporateclients.findOne({
            where: {
                state: 1,
                corporateclients_id: doc.corporateclients_id
            }
        })
        if (orporateclients.corporateclients_type) {
            queryStr = `select m.*,ptd.suggest_price,0 as apply_number from tbl_erc_materiel m 
            left join tbl_erc_producepricetemplatedetail ptd on (ptd.materiel_id = m.materiel_id and ptd.state = 1 and producepricetemplate_id=?)
            where m.state=1 and m.domain_id = ?`;
            replacements = [orporateclients.corporateclients_type, user.domain_id];
        } else {
            queryStr = `select m.*,0 as apply_number from tbl_erc_materiel m 
            where m.state=1 and m.domain_id = ?`;
