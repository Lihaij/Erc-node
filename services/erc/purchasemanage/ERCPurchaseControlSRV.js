...
const tb_purchaseapplydetail = model.erc_purchaseapplydetail; //采购单申请表详情
...
const RecordingVoucherscnew = require('../../../util/RecordingVouchersc');
...
async function allDemand(params) {
    try {
        ...
        queryStr = `
            SELECT
                om.materiel_id  ,sum(om.materiel_amount) AS materiel_amount 
            FROM
                tbl_erc_ordermateriel om ,tbl_erc_order o ,tbl_erc_materiel m
            WHERE
                om.state = 1 AND o.state = 1 AND m.state = 1
                AND om.order_id = o.order_id AND om.materiel_id = m.materiel_id 
                AND o.domain_id = ${ params.nowDoaminId }
                AND o.order_review_date >= '${params.beginTime}' AND o.order_review_date <= '${params.endTime}'
                AND o.order_review_state = 2 AND om.mrp_state = 0 AND m.materiel_manage = 1 GROUP BY om.materiel_id`;
        let resultAQ = await sequelize.query(queryStr, {
            replacements: [],
            type: sequelize.QueryTypes.SELECT
        });
        if (resultAQ && resultAQ.length > 0) {
            for (let r of resultAQ) {
                insertDate.push({
        ...
        queryStr = `
            SELECT
                om.materiel_id ,om.order_id ,sum(om.materiel_amount) AS materiel_amount 
            FROM
                tbl_erc_ordermateriel om ,tbl_erc_order o ,tbl_erc_materiel m
            WHERE
                om.state = 1 AND o.state = 1 AND m.state = 1
                ...
                AND o.domain_id = ${ params.nowDoaminId }
                AND o.order_review_date >= '${params.beginTime}' AND o.order_review_date <= '${params.endTime}'
                AND o.order_review_state = 2 AND om.mrp_state = 0 AND m.materiel_manage = 2 GROUP BY om.materiel_id,om.order_id`;
        let resultDD = await sequelize.query(queryStr, {
            replacements: [],
            type: sequelize.QueryTypes.SELECT
        });
        if (resultDD && resultDD.length > 0) {
            for (let r of resultDD) {
                insertDate.push({
                    materiel_id: r.materiel_id,
                    order_id: r.order_id,
                    demand_amount: r.materiel_amount,
        ...
        if (insertDate.length > 0) {
            await tb_alldemand.bulkCreate(insertDate);
        }

        queryStr = `update tbl_erc_ordermateriel om,tbl_erc_order o
        set om.mrp_state = 1
...
async function netDemand(params) {
    try {
        ...
        */
        let queryStr, replacements = [],
            netDemandAmount = 0;
        //获取本次，本机构mrp运算时，需要的安全库存量
        queryStr = `select a.materiel_id from tbl_erc_alldemand a,tbl_erc_materiel m 
            where a.state=1 and m.state=1 and a.materiel_id=m.materiel_id
            and a.mrp_domain_id=${params.nowDoaminId}
            and a.mrp_state=0 
            group by materiel_id`
        let materielIDS = await sequelize.query(queryStr, {
            replacements: [],
            type: sequelize.QueryTypes.SELECT
        });
        stockmapSafeTemp = []
        for (let m of materielIDS) {
            queryStr = `select materiel_id,sum(current_amount) as sum_current_amount
                from tbl_erc_stockmap
                where state=1 and materiel_id=${m.materiel_id} and storage_type=1`
            let resultStockmap = await sequelize.query(queryStr, {
                replacements: [],
                type: sequelize.QueryTypes.SELECT
            });
            stockmapSafeTemp.push({
                materiel_id: resultStockmap[0].materiel_id,
                sum_current_amount: resultStockmap[0].sum_current_amount
            })
        }
...
async function production(params) {
    try {
        ...
        for (let nd of netDemand) {
            ...
            // materiel_source 1自制，2外购，3委外加工，4受托
            ...
            // ****************************最终产品生产任务****************************
            // 成品生产任务单
            // let POID = await genBizCode(CODE_NAME.SCRW, params.nowDoaminId, 6);
            ...
            let addProductTask = await tb_productivetask.create({
                ...
                change_state: 0,
                outsource_sign: nd.materiel_source == 1 || nd.materiel_source == 4 ? nd.materiel_source : 3, //委外标志  1自制   3委外
                biz_code: biz_code
            });

            //成品生产任务单---工序
            queryStr = `select ppp.procedure_id,pp.department_id,ppp.priority from tbl_erc_productplanprocedure ppp
                left join tbl_erc_productionprocedure pp on (ppp.procedure_id = pp.procedure_id and pp.state = 1)
                where ppp.product_plan_id = ${productPlan.product_id} and ppp.rlt_materiel_code = ${nd.materiel_id}`
            let pppLasts = await sequelize.query(queryStr, {
                replacements: [],
                type: sequelize.QueryTypes.SELECT
            });
            for (let pppLast of pppLasts) {
                let addProductiveTaskProcess = await tb_productivetaskprocess.create({
                    productivetask_id: addProductTask.productivetask_id,
                    productivetask_code: addProductTask.productivetask_code,
                    department_id: nd.materiel_source != 3 ? pppLast.department_id : '',
                    procedure_id: pppLast ? pppLast.procedure_id : '',
                    procedure_level: pppLast ? pppLast.priority : ''
                });
            }
            //成品生产任务单---投料
            ...
            for (let pd of productPlanDetail) {
                ...
                if (netDemandAmount > 0) {
                    // materiel_source 1自制，2外购，3委外加工，4受托
                    // 采购的物料加入净需求
                    if (pd.materiel_source == 2) { //外购并且是按订单号管理的物料，放入净需求
                        ...
                        addProductTask = await tb_productivetask.create({
                            ...
                            productivetask_state: 1,
                            change_state: 0,
                            outsource_sign: pd.materiel_source == 1 || pd.materiel_source == 4 ? pd.materiel_source : 3, //委外标志  1自制   2委外
                            biz_code: biz_code
                        });
                        ...
                        queryStr = `select ppp.procedure_id,pp.department_id,ppp.priority from tbl_erc_productplanprocedure ppp
                            left join tbl_erc_productionprocedure pp on (ppp.procedure_id = pp.procedure_id and pp.state = 1)
                            where ppp.product_plan_id = ${productPlan.product_id} and ppp.rlt_materiel_code = ${pd.src_materiel_id}`
                        let pppResolves = await sequelize.query(queryStr, {
                            replacements: [],
                            type: sequelize.QueryTypes.SELECT
                        });
                        for (let pppResolve of pppResolves) {
                            let addProductiveTaskProcess = await tb_productivetaskprocess.create({
                                productivetask_id: addProductTask.productivetask_id,
                                productivetask_code: addProductTask.productivetask_code,
                                department_id: pd.materiel_source != 3 ? pppResolve.department_id : '',
                                procedure_id: pppResolve ? pppResolve.procedure_id : '',
                                procedure_level: pppResolve ? pppResolve.priority : ''
                            });
                        }
                        //投料生产任务单---投料
                        for (let pdTemp of productPlanDetail) {
...
async function getDPtheoryNumOne(ptps, DPtheoryNumAll, departmentAll) {

    let DPtheoryNumOne = DPtheoryNumAll.find(e => {
        if (e.department_id == ptps.department_id &&
            e.procedure_id == ptps.procedure_id) {
            return e
        }
    });
    let departmentOne = departmentAll.find(e => {
        if (e.department_id == ptps.department_id) {
            return e
        }
    });
    ...
    //如果没找到产能信息，则获取该工序的默认产能
    if (!DPtheoryNumOne || Number(DPtheoryNumOne.sumDayCapacityTheory) == 0) {
        queryStr = `select * from tbl_erc_productionprocedure where state=1 and procedure_id=${ptps.procedure_id}`
        if (ptps.procedure_id == 409) {
            logger.info(1111)
        }
        let procedureNow = await sequelize.query(queryStr, {
            replacements: [],
            type: sequelize.QueryTypes.SELECT
        });

        // if (procedureNow[0].default_capacity) {
        DPtheoryNumOne = {
            department_id: ptps.department_id,
            department_name: departmentOne.department_name,
            procedure_id: ptps.procedure_id,
            procedure_name: procedureNow[0].procedure_name,
            sumDayCapacityF: "0",
            sumDayCapacityTheory: procedureNow[0].default_capacity,
            sumDayCapacityZ: "0"
        }
        // }
    }
    logger.info(DPtheoryNumOne)
    return DPtheoryNumOne
}
//----------------------------------------排产-----------------------------------------
    ...
    */
    try {
        ...
        for (let ptgo of PTGroupOrderRes) {
            ...
            // //生产任务单（tbl_erc_productivetask）与工序（tbl_erc_productivetaskprocess）一对多关系
            // queryStr = `select
            //     pt.productivetask_id,pt.productivetask_code,pt.product_id,pt.materiel_id,pt.domain_id,pt.product_level,
            //     pt.taskdesign_number,pt.stock_in_number,pt.stock_in_state,pt.stock_out_number,pt.stock_out_state,pt.order_id,
            //     pt.productivetask_state,pt.taskdesign_price,pt.change_state,pt.outsource_sign,pt.materiel_code,
            //     ptp.department_id,ptp.procedure_id,ptp.procedure_level
            //     from tbl_erc_productivetask pt
            //     left join tbl_erc_productivetaskprocess ptp on (pt.productivetask_id=ptp.productivetask_id)
            //     where pt.state=1 and productivetask_state=1 and outsource_sign=1
            //     and domain_id=${params.nowDoaminId} and order_id='${ptgo.order_id}'
            //     order by pt.product_level`
            // let PTByOrder = await sequelize.query(queryStr, {
            //     replacements: [],
            //     type: sequelize.QueryTypes.SELECT
            // });
        ...
        for (let ptgo of PTGroupOrder) { //order
            logger.info(ptgo)
            //生产任务单按级别倒叙生产
            for (let pi of ptgo.pt_item) { //leave
                if (pi.level == '') {
                    break
                }
                //将生产任务单排产
                for (let task of pi.levelItem) { //task

                    let productivetaskprocess = await tb_productivetaskprocess.findAll({
                        attributes: [
                            'department_id',
                            'procedure_id',
                            'procedure_level'
                        ...
                        where: {
                            productivetask_id: task.productivetask_id
                        }
                    })

                    let theoryProcessArray = []
                    for (ptps of productivetaskprocess) {
                        let theroy = await getDPtheoryNumOne(ptps, DPtheoryNumAll, departmentAll)
                        theoryProcessArray.push(theroy)
                    }
                    logger.info(theoryProcessArray)

                    let theoryProcessArrayFinal = theoryProcessArray.filter(item => {
                        return item.sumDayCapacityTheory != 0
                    })
                    theoryProcessArrayFinal.sort(function (a, b) {
                        return a.sumDayCapacityTheory > b.sumDayCapacityTheory ? 1 : -1
                    });
                    let minTheroyProcess = theoryProcessArrayFinal[0].sumDayCapacityTheory

                    for (ptps of productivetaskprocess) { //task<->process
                        let pt = {
                            ...task.dataValues,
                            ...ptps.dataValues
                        }
                        logger.info(pt)
                        let DPtheoryNumOne = await getDPtheoryNumOne(ptps, DPtheoryNumAll, departmentAll)
                        DPtheoryNumOne.sumDayCapacityTheory = minTheroyProcess
                        ...
                        //         type: sequelize.QueryTypes.SELECT
                        //     });
                        //     queryStr = `select * from tbl_erc_department where state=1 and department_id='${ptps.department_id}'`
                        //     let departmentNow = await sequelize.query(queryStr, {
                        //         replacements: [],
                        //         type: sequelize.QueryTypes.SELECT
                        //     });
                        ...
                        //         DPtheoryNumOne = {
                        //             department_id: ptps.department_id,
                        //             department_name: departmentNow[0].department_name,
                        //             procedure_id: ptps.procedure_id,
                        //             procedure_name: procedureNow[0].procedure_name,
                        //             sumDayCapacityF: "0",
                        //             sumDayCapacityTheory: procedureNow[0].default_capacity,
                    ...
                    queryStr = `select min(ppmaster_date) as min_ppmaster_date,max(ppmaster_date) as max_ppmaster_date 
                    from tbl_erc_ppmaster where state=1 and productivetask_code='${task.productivetask_code}'`
                    let dateResult = await sequelize.query(queryStr, {
                        replacements: [],
                        type: sequelize.QueryTypes.SELECT
                    });
                    ...
                    if (dateResult.length > 0) {
                        await tb_productivetask.update({
                            ppmaster_begin_time: dateResult[0].min_ppmaster_date,
                            ppmaster_end_time: dateResult[0].max_ppmaster_date
                        }, {
                            where: {
                                productivetask_code: task.productivetask_code
                            }
                        });
                    }
                }
...
//----------------------------------------净需求生成采购申请-----------------------------------------
async function purchaseApply(params) {
    try {
        let queryStr, replacements = []

        queryStr = `select n.*,m.materiel_sale,m.materiel_manage from tbl_erc_netdemand n,tbl_erc_materiel m
            where n.state=1 and m.state=1 and n.materiel_id=m.materiel_id 
            and n.mrp_state=0 and n.mrp_domain_id=${params.nowDoaminId} and m.materiel_source=2`;
        let result = await sequelize.query(queryStr, {
            replacements: [],
        ...
        if (result && result.length > 0) {
            let applyID = await Sequence.genPurchaseApplyID(params.nowDoaminId);
            let biz_code = await genBizCode(CODE_NAME.CGSQ, params.nowDoaminId, 6);
            let addPurchaseapply = await tb_purchaseapply.create({
                purchaseapply_id: applyID,
                app_domain_id: params.nowDoaminId,
                apply_approver: '',
                apply_applicant: '',
                apply_state: 2,
                biz_code: biz_code,
            ...
            for (let r of result) {
                let addPurchaseApplyDetail = await tb_purchaseapplydetail.create({
                    ...
                    order_id: r.order_id,
                    materiel_id: r.materiel_id,
                    apply_number: r.netdemand_amount,
                    apply_money: r.materiel_sale * r.netdemand_amount
                });
            }
        }
...
async function checkInventory(allDemand) {
    ...
    if (allDemand.materiel_manage == 2) {
        /*
            销售订单管理的物料，查找闲置库存，并且用订单号将占用的物料做标记
            storage_type == 3表示闲置库存
        */
        queryStr = `select s.* from tbl_erc_stockmap s,tbl_erc_warehouse w 
            where s.state=1 and w.state=1 
            and s.warehouse_id=w.warehouse_id  
            and w.domain_id=${allDemand.nowDoaminId} and s.storage_type = 3 and s.materiel_id='${allDemand.materiel_id}' 
            and w.warehouse_name not in ('不良品仓','废品仓')`;
        let resultStockmap = await sequelize.query(queryStr, {
            replacements: [],
            type: sequelize.QueryTypes.SELECT
        });
            ...
            for (let sm of resultStockmap) {
                if (allDemand.demand_amount - alreadyNum < sm.current_amount) {
                    await tb_stockmap.update({
                        ...
                        available_amount: allDemand.demand_amount - alreadyNum,
                        storage_type: 2
                    }, {
                        where: {
                    ...
                    await tb_stockmap.create({
                        ...
                        available_amount: sm.current_amount - (allDemand.demand_amount - alreadyNum),
                        materiel_bar_code: '',
                        is_idle_stock: 1,
                        storage_type: 3
                    });
        ...
        /*
            安全库存管理的物料，查找安全库存
        */
        let have = stockmapSafeTemp.find((item) => {
            return item.materiel_id == allDemand.materiel_id
        })
        if (have) {
            if (Number(allDemand.demand_amount) <= Number(have.sum_current_amount)) {
                have.sum_current_amount = Number(have.sum_current_amount) - Number(allDemand.demand_amount)
            } else {
                have.sum_current_amount == 0
                netDemandAmount = Number(allDemand.demand_amount) - Number(have.sum_current_amount)
            }
        } else {
            //获取本次，本机构mrp运算时，需要的安全库存量
            queryStr = `select materiel_id,sum(current_amount) as sum_current_amount
                from tbl_erc_stockmap
                where state=1 and materiel_id=${allDemand.materiel_id} and storage_type=1`
            let resultStockmap = await sequelize.query(queryStr, {
                replacements: [],
                type: sequelize.QueryTypes.SELECT
            });

            if (Number(allDemand.demand_amount) <= Number(resultStockmap[0].sum_current_amount)) {
                resultStockmap[0].sum_current_amount = Number(resultStockmap[0].sum_current_amount) - Number(allDemand.demand_amount)
            } else {
                resultStockmap[0].sum_current_amount == 0
                netDemandAmount = Number(allDemand.demand_amount) - Number(resultStockmap[0].sum_current_amount)
            }
            stockmapSafeTemp.push({
                materiel_id: resultStockmap[0].materiel_id,
                sum_current_amount: resultStockmap[0].sum_current_amount
...
async function getAllDemand(req, res) {
    try {
        ...
        queryStr += ` order by mrp_date desc`
...
async function getPurchaseOrder(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = `select p.biz_code,p.purchaseorder_id,s.supplier_name,
            s.supplier,p.created_at,d.domain_name as order_domain,dd.domain_name as purchase_domain,
            p.purchaseorder_state
        ...
        queryStr += ' order by purchaseorder_id desc';
        let result = await common.queryWithCount(sequelize, req, queryStr, replacements);
        returnData.total = result.count;
        returnData.rows = [];
        for (let r of result.data) {
...
async function addPuchaseApply(req, res) {
    ...
    try {
        ...
        const biz_code = await genBizCode(CODE_NAME.CGSQ, user.domain_id, 6);
...
async function getPuchaseApply(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = `select a.purchaseapply_id,a.apply_state,a.approval_date,a.created_at,a.biz_code,
             ap.username as apply_applicant,av.username as apply_approver,a.description,a.order_type,a.data_source 
...
async function initAct(req, res) {
    try {
        ...
        returnData.purchaseOrderState = GLBConfig.PURCHASEORDERSTATE; //采购单状态
...
async function dataExtract(req, res) {
    try {
        ...
        for (let d of domain) {
            ...
            params.logName = moment().format('YYYY-MM-DD') + '---' + d.domain_id
...
async function deleteData(req, res) {
    let result
    queryStr = "update tbl_erc_ordermateriel set mrp_state = 0"
    ...
    queryStr = "delete from tbl_erc_productivetaskprocess"
    result = await sequelize.query(queryStr, {
        replacements: [],
        type: sequelize.QueryTypes.DELETE
    });
    ...
    // const rv = require('../../../util/RecordingVouchersc')
    // let aaa = await rv.getSalary(326, 3000)
    // logger.info(aaa)
    // common.sendData(res, {});

}

async function testRC(req, res) {
    try {
        await sequelize.transaction(async (transaction) => {
            await RecordingVoucherscnew.createRecordingVoucherSC('CGRK', req.user, {
                supplier_id: 83,
                // productivetask_id: 561,
                materiel_id: 376
                // department_id: 'BM0002',
                // other_main_id: "QT2"
            }, transaction)
...
async function modifyPuchareOrderState(orderState, description, purchaseOrderId, orderApprover) {
    if (orderState == 2) {
        let purchasedetail = await tb_purchasedetail.findAll({
            where: {
                state: 1,
        ...
        for (let pd of purchasedetail) {
            let queryStr = `update tbl_erc_purchaseapplydetail set apply_number_done = apply_number_done - ? 
                where purchaseapplydetail_id = ?`
            let result = await sequelize.query(queryStr, {
                replacements: [pd.purchase_number, pd.source_apply_id],
                type: sequelize.QueryTypes.UPDATE
            });
        }
    ...
    await tb_purchaseorder.update({
        purchaseorder_state: orderState,
        approval_date: new Date(),
        purchase_approver: orderApprover,
...
exports.modifyPuchareOrderState = modifyPuchareOrderState;